...
> MOI: the `seed` is to fix a random process. For example, if you randomly draw a number between 1 to 10, every time you run it, you will get a different number. However, if you set the `seed` you will allways get the same result. To see that, you can comment the `set.seed(1)`, run the chunk multiple times (to see that you get different trees), and then uncoment the `set.seed(1)` and run again multipe times (to see taht you allways get the same result).
